...
void HMM::dump_model(char* filename)
{
 ...
 // static int previousStateSeq[1000];
  //static int currentStateSeq[1000];
  static vector<int> previousStateSeq;
   vector<int> currentStateSeq;
  double maxProb;int maxProbState;
...
/*  cout<<"---------------printing the optimal sequence--------------------"<<endl;
	for (i = 0; i < global_symbol_count; i++) { 
    	...
    	cout<< maxProbState <<" ";
    	currentStateSeq.push_back(maxProbState);

    }
    if(currentStateSeq == previousStateSeq)
    {
    	STOPITER = 1;
    	cout<<"----------------convergence occured-------------------"<<endl;
    }
...
int HMM::load_string_matrix(char* filename)
{
  ...
  // load values into matrix from the test.hmm.seq file
  diff_len=FALSE; //means diff lenght is equal to 0
